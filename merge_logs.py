                        ...
                        try:
                            ...
                            if isinstance(obj.get("input"), str):
                                try:
                                    ...
                                    print(
                                        f"⚠️ Failed to parse 'input' JSON string: {line}"
                                    )
                                    continue
                            ...
                            if isinstance(out_field, str):
                                try:
                                    ...
                                    print(
                                        f"⚠️ Failed to parse 'output' JSON string: {line}"
                                    )
                                    continue
                            ...
                            for msg in msgs:
                                if msg.get("type") == "human" and msg.get(
                                    "name", ""
                                ) not in ["retrieve_docs", "execute_sparql_query"]:
                            ...
                            if first_human_msg and first_ai_response:
                                conv_signature = (
                                    f"{first_human_msg}:::{first_ai_response}"
                                )
                            ...
                            question: str = ""
                            for msg in msgs:
                                # NOTE: avoid using the messages like "Fix the SPARQL query" as question
                                if (
                                    msg.get("type") == "human"
                                    and msg.get("name", "") != "retrieve_docs"
                                    and "Fix the SPARQL query helping yourself with the error message and context from previous messages"
                                    not in msg.get("content", "")
                                    and "Please fix the query, and try again."
                                    not in msg.get("content", "")
                                ):
                                ...
                                if msg.get("type") == "ai":
                                    try:
                                        query_results = extract_and_exec_sparql(
                                        ...
                                        if query_results:
                                            ...
                                            msg["query_results"] = (
                                                query_results.model_dump()
                                            )
                                        else:
                                            counts["msgs_no_results"] += 1
                                    except Exception as e:
                                        print(
                                            f"⚠️ Failed to extract/execute SPARQL: {e}"
                                        )
